...
public:
    ...
    {
        for (auto link = this->links_.begin(); link != this->links_.end(); link++)
        {
    ...
    void initTree(std::map<std::string, std::string>& parent_link_tree)
    {
        ...
        for (auto joint = this->joints_.begin(); joint != this->joints_.end(); joint++)
        {
            ...
            {
                throw ParseError("Joint [" + joint->second->name +
            ...
            else
            {
                ...
                if (!parent_link)
                {
                    throw ParseError("parent link [" + parent_link_name + "] of joint [" + joint->first +
                                     "] not found.  This is not valid according to the URDF spec. Every link you refer "
                                     "to from a joint needs to be explicitly defined in the robot description. To fix "
                                     "this problem you can either remove this joint [" +
